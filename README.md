...
Containing every style rule and every (safe) set of parameters that Unity Style Sheets, this model aims to be compliant to the USS Specification and, where necessary, the CSS3 specification that the import-export .NET library **ExCSS** abides by, which is used by Unity Technologies for the import
