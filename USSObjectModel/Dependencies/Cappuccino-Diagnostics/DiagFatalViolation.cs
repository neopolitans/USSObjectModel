...
namespace Cappuccino
{
    namespace Core
    {
        ...
        public static partial class Diag
        {
            ...
            public static void FatalViolation(string context)
            {
                if (context == null)
                {
                    Violation("Violation of Diag.TranslatorViolation(string context) -> no context message for violation.");
                    return;
                }
                ...
                Debug.LogError($"[Cappuccino] A fatal violation has occured.\n {context} \n" +
            ...
            public static void FatalViolation(string context, int diagnosticsFrame)
            {
                if (context == null)
                {
                    Violation("Violation of Diag.TranslatorViolation(string context, int diagnosticsFrame) -> no context message for violation.");
                    return;
                }

                if (diagnosticsFrame < 0)
                {
                    Violation("Violation of Diag.TranslatorViolation(string context, int diagnosticsFrame) -> diagnosticsFrame is less than 0.");
                    return;
                }
